...
#ifdef SMMOD
	m_ViewModelArm = CreateEntityByName("viewmodelarms");
	m_ViewModelArm->SetModel("models/weapons/v_hands.mdl");
	m_ViewModelArm->SetSolid(SOLID_NONE);
	//m_ViewModelArm->AddEffects(EF_BONEMERGE); //Handled per weapon
	m_ViewModelArm->AddEffects(EF_NOSHADOW);
	m_ViewModelArm->SetViewOffset(GetViewModel()->GetViewOffset());
	m_ViewModelArm->FollowEntity(GetViewModel(), false);
	m_ViewModelArm->SetOwnerEntity(this);
#endif
	//
...
#ifdef SMMOD
	if (pWeapon->HasDynamicArms())
	{
		m_ViewModelArm->AddEffects(EF_BONEMERGE);
	}
	else
	{
		m_ViewModelArm->RemoveEffects(EF_BONEMERGE);
	}
#endif
